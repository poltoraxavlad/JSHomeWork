...
const campaignName = constants.campaignNameForNoRedirect.forexTB;
const randomEmail = userData.getRandomEmail();

console.log("Lead email for ForexTB(no redirect pixel) =", randomEmail);

const button = element(by.id("lead-form-submit"));
const popup = element(by.xpath('//div[@class="nrp__block"]'));
const funnelURL = "https://mtxcapital.com/forextb_no_redirect_voluum";

let volumeToken, campaignUrl, conversions, visits, cpid;

describe('Check no redirect volume for ForexTB Brand(Scipio CRM)', function () {
