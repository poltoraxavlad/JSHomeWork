...
const campaignName = constants.campaignNameForNoRedirect.capitalix;
const randomEmail = userData.getRandomEmail();

console.log("Lead email for Capitalix(no redirect pixel) =", randomEmail);

const button = element(by.id("lead-form-submit"));
const popup = element(by.xpath('//div[@class="nrp__block"]'));
const funnelURL = "https://mtxcapital.com/capitalix_no_redirect_voluum";

let volumeToken, campaignUrl, conversions, visits, cpid;

describe('Check no redirect volume for Capitalix Brand(NG CRM)', function () {
    ...
    it('Get funnel URL',function () {
        ...
        it('7168: Create lead no autologin', async function () {
            await registerStepsHelper.registerDataForOneStepFunnelWithoutPassword(randomEmail, constants.country.arabic);
