...
const campaignName = constants.campaignNameForNoRedirect["101investing"];
const randomEmail = userData.getRandomEmail().replace('e2e', '');

console.log("Lead email for 101investing(no redirect pixel) =", randomEmail);

const button = element(by.id("lead-form-submit"));
const popup = element(by.xpath('//div[@class="nrp__block"]'));
const funnelURL = "https://mtxcapital.com/101investing_no_redirect_voluum";

let volumeToken, campaignUrl, conversions, visits, cpid;

describe('Check no redirect volume for 101investing Brand(Finosoft CRM)', function () {
